...
public sealed partial class MauiWebView
{
    ...
    private static ILogger Log => _log ??= MauiDiagnostics.LoggerFactory.CreateLogger<MauiRootPage>();
    ...
    public MauiWebView()
    {
        ...
        BlazorWebView.RootComponents.Add(
            new RootComponent
            {
                ComponentType = typeof(MauiRouter),
                Selector = "#app",
                Parameters = MauiSettings.ClientAppParameters,
            });
        lock (StaticLock)
            _current = this;
    ...
    public void SetScopedServices(IServiceProvider scopedServices)
    {
